    <!-- Button aligned to the right -->
    <div class="row justify-content-end mb-3">
        <div class="col-5 d-flex justify-content-end">
            <button @click="showAdd = true; $wire.save_image('itemSaved');" class="btn btn-outline-primary me-2 w-50">Save Item</button>
            <button @click="showAdd = true;$wire.clearLogo();" class="btn btn-outline-success w-50">Back</button>
        </div>
    </div>


    <!-- Underline hr -->
    <hr class="border-2 border-top border-dark">

    <div class="scroll-container" id="scrollContainer">
        <div class="scroll-content">
            <div class="mt-1"
                x-data="{
                    name: $wire.entangle('name_'),
                    cost: $wire.entangle('cost_'),
                    bgColor: $wire.entangle('bgColor_'),
                    ingredients: $wire.entangle('ingredients_'),
                    addons: $wire.entangle('addons_'),
                    logo: $wire.entangle('logo_'),
                    selectedImage:$wire.entangle('selectedImage'),
                    selectedImageUrl: null,  // Initialize selected image URL
                    previewImage(event) {
                        const file = event.target.files[0];
                        if (file) {
                            this.selectedImageUrl = URL.createObjectURL(file);  // Generate a temporary preview URL
                        }
                    },
                    getLogoUrl(filename) {
                        return filename ? '/custom/app/img/clients/{{ Auth::user()->companyID }}/' + filename : '';
                    },
                    toggleIngredient(id) {
                        if (this.ingredients[id] !== undefined) {
                            delete this.ingredients[id];
                        } else {
                            this.ingredients[id] = 0;
                        }
                    },
                    toggleAddon(id) {
                        if (this.addons.includes(id)) {
                            this.addons = this.addons.filter(aid => aid !== id);
                        } else {
                            this.addons.push(id);
                        }
                    },
                  save() {
                    //remove null from array
                    let filteredObject = {};
                    for (let key in this.ingredients) {
                        if (this.ingredients[key] !== null) {
                            filteredObject[key] = this.ingredients[key];
                        }
                    }
                    //
                    let addOnsArray=[];
                    addOnsArray=Object.values(this.addons);
                    const jsonString = {
                        name: this.name,
                        cost: this.cost,
                        bgColor: this.bgColor,
                        logo: this.logo,
                        ingredients: filteredObject,
                        addons: this.addons.length ? {addons:addOnsArray } : null
                    };
                    $wire.save_item(jsonString);
                },
                    reset() {
                        this.name = '';
                        this.cost = 0;
                        this.bgColor = '';
                        this.logo = '';
                        this.ingredients = {};
                        this.addons = [];
                        $wire.resetFields();
                    },
                }"
             >
                
                <!-- Row 1: Name and Cost -->
                <div class="row row-divider">
                    <div class="col-9">
                        <div class="form-floating">
                            <input type="text" class="form-control" x-model="name" placeholder="Enter name">
                            <label>Name</label>
                        </div>
                    </div>
                    <div class="col-3">
                        <div class="form-floating">
                            <input type="number" class="form-control" x-model="cost" placeholder="0.00">
                            <label>Cost</label>
                        </div>
                    </div>
                </div>
                
                <!-- Row 2: Background Color and Logo -->
                <div class="row row-divider">
                    <div class="col-9">
                        <label>Background Color:</label><br>
                        <div class="d-flex flex-wrap">
                            <template x-for="color in ['bg-primary', 'bg-secondary', 'bg-success', 'bg-danger', 'bg-warning', 'bg-info', 'bg-light', 'bg-dark']" :key="color">
                                <div :class="['color-box', color]" @click="$wire.set('bgColor_', color)" style="position: relative; text-align: center; font-size: 24px; font-weight: bold; cursor: pointer; display: flex; align-items: center; justify-content: center; width: 50px; height: 50px;">
                                    <span x-show="bgColor === color" :style="{'color': (color === 'bg-dark' ? '#fff' : '#000'), 'font-size': '40px', 'font-weight': 'bold', 'line-height': '50px'}">X</span>
                                </div>
                            </template>
                        </div>
                    </div>
                    <!-- **** Image Copy begins here --- ***-->
                    <div class="col-3" x-data="{ openFileInput: false }">
                        <label>Logo:</label><br>

                        <button class="btn btn-secondary"
                            @click="openFileInput = true; 
                            $nextTick(() => document.getElementById('item-upload').click())" >
                             Upload Logo
                        </button>
                         <!-- Hidden file input bound to Livewire -->
                        <!-- DO NOT forget to change id -->
                         <input
                            type="file" 
                            id="item-upload" 
                            wire:model="selectedImage" 
                            class="d-none"
                            @change="selectedImageUrl = URL.createObjectURL($event.target.files[0])"
                         >

                        <!-- Smooth transition between predetermined logo and selected image -->
                        <div class="relative mt-2" style="max-width: 100px;">
                            <!-- Show new uploaded image -->
                            <img x-show="selectedImageUrl"
                                :src="selectedImageUrl"
                                class="img-thumbnail absolute inset-0 w-full h-full transition-opacity duration-500 ease-in-out"
                                x-transition:enter="opacity-0"
                                x-transition:enter-end="opacity-100"
                                x-transition:leave="opacity-100"
                                x-transition:leave-end="opacity-0"
                            >

                            <!-- Show predetermined image -->
                            <img x-show="!selectedImageUrl"
                                :src="getLogoUrl(logo)"
                                class="img-thumbnail w-full h-full transition-opacity duration-500 ease-in-out"
                                x-transition:enter="opacity-0"
                                x-transition:enter-end="opacity-100"
                                x-transition:leave="opacity-100"
                                x-transition:leave-end="opacity-0"
                            >
                        </div>
                        <!-- ***** Image copy ends here -->
                    </div>
                </div>
                
                <!-- Row 3: Ingredients and Addons -->
                <div class="row">
                    <!-- Ingredients -->
                    <div class="col-7">
                        <label>Ingredients:</label>
                        <table class="table table-bordered">
                            @foreach ($ingredients as $ingredient)
                                <tr>
                                    <td style="display: nonex;">{{ $ingredient->id }}</td>
                                    <td>
                                        <button class="btn square-btn btn-outline-secondary" 
                                                :class="ingredients[{{ $ingredient->id }}] !== undefined ? 'btn-dark' : ''" 
                                                x-text="ingredients[{{ $ingredient->id }}] !== undefined ? 'X' : ' '" 
                                                @click="toggleIngredient({{ $ingredient->id }})"></button>
                                    </td>
                                    <td>{{ $ingredient->name }}</td>
                                    <td>
                                        <input type="number" 
                                               x-model="ingredients[{{ $ingredient->id }}]" 
                                               class="form-control" 
                                               :disabled="ingredients[{{ $ingredient->id }}] === undefined">
                                    </td>
                                </tr>
                            @endforeach
                        </table>
                    </div>
                    
                    <!-- Addons -->
                    <div class="col-5">
                        <label>Addons:</label>
                        <table class="table table-bordered">
                            @foreach ($addons as $addon)
                                <tr>
                                    <td style="display: none;">{{ $addon->id }}</td>
                                    <td>
                                        <button class="btn square-btn btn-outline-secondary" 
                                                :class="addons.includes({{ $addon->id }}) ? 'btn-dark' : ''" 
                                                x-text="addons.includes({{ $addon->id }}) ? 'X' : ' '" 
                                                @click="toggleAddon({{ $addon->id }})"></button>
                                    </td>
                                    <td>{{ $addon->name }}</td>
                                </tr>
                            @endforeach
                        </table>
                    </div>
                </div>
                
                <!-- Save Button -->
                <div class="row">
                    <div class="col-6">
                        <button class="btn btn-success w-100" @click="save">Save</button>
                    </div>
                    <div class="col-6">
                        <button class="btn btn-danger w-100" @click="reset">Reset</button>
                    </div>
                </div>
            </div>
        </div>
    </div>
    @script
        <script>
             Livewire.on('itemSaved', () => {
                alert('Success! Image saved successfully.');
             });
             Livewire.on('error_itemSaved', () => {
                alert('Error! Image not saved.');
             });
        </script>
    @endscript